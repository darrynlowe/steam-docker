...
It has some additional complexity because:
- I'm running an Nvidia 1080 Ti which is now not supported by the latest Nvidia drivers. So there's some butchery from the AUR to bring down the legacy driver set inside the container since it's needed for dependencies. This can create a situation with version mismatch with the host, so make sure the host is also up to date. So if you don't need this, you could strip back the Dockerfile quite considerably.
- I'm using a host with multiple seats, so the /run/seatd.sock and Xorg -seat parameter would not be needed if you're not
- I'm running it in privilidged mode for debugging. I think it's pretty close to running without priviledges though, but that's a TODO
